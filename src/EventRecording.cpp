...
void validateEvents(const std::vector<StreamEvent>& events) {
    require(!events.empty(), "missing run");
    require(std::holds_alternative<RunStarted>(events.front().payload), "first event must start the run");
    const RunId runId = events.front().identity.runId;
    require(runId != 0, "run ID must be nonzero");
    std::uint64_t generation = 0;
    float lastTime = 0;
    bool finished = false;
    bool acceptingStarts = true;
    std::map<SensorId, SensorState> sensors;
    for (std::size_t i = 0; i < events.size(); ++i) {
        const auto& e = events[i];
        const auto& h = e.identity;
        require(!finished, "events after run finish");
        require(h.runId == runId && h.streamSequence == i + 1, "run ID or stream sequence mismatch");
        require(finite(h.timeSeconds) && h.timeSeconds >= 0, "invalid timestamp");
        if (std::holds_alternative<RunReset>(e.payload)) {
            require(i > 0 && h.runGeneration == generation + 1 && h.timeSeconds == 0, "invalid run reset");
        ...
        if (std::holds_alternative<RunStarted>(e.payload)) {
            require(i == 0 && generation == 0 && h.timeSeconds == 0, "duplicate/invalid run start");
    ...
    require(finished, "missing RUN_FINISHED (truncated or incomplete run)");
}
